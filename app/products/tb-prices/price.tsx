...
import { DropdownMenuLst } from "@/components/dropdown";
import { TableProductsClientProps } from "@/app/products/types";
import { useState, useEffect } from "react";

const Price = ({ products }: TableProductsClientProps) => {
  const [isProdNull, setProd] = useState(true);
  useEffect(() => {
    if (products.length ===0) setProd(true)
  }, [])

 // console.log(products.map(p => p.product.length));
